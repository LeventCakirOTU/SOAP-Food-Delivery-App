...
public class RegisterDriverScreen extends JPanel {
    ...
    private final JTextField vehicleField;

    public RegisterDriverScreen(MainFrame frame, UserService userService) {
        ...
        JLabel title = UIHelper.label("Register as Driver", AppColors.FONT_HEADING, AppColors.ACCENT);
        ...
        vehicleField  = UIHelper.styledField("Vehicle info (e.g. Toyota 2019)");

        for (JComponent f : new JComponent[]{nameField, emailField, passwordField, vehicleField}) {
            f.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));
        }

        JButton submitBtn = UIHelper.primaryButton("Register");
        ...
        addLabeledField(card, "Vehicle Info", vehicleField);
        card.add(Box.createVerticalStrut(20));
        card.add(submitBtn);
    ...
    private void handleRegister(MainFrame frame, UserService userService) {
        ...
        if (name.isEmpty() || email.isEmpty() || pass.isEmpty() || vehicle.isEmpty()) {
            UIHelper.showError(this, "All fields are required");
            return;
        }

        userService.registerDriver(name, email, pass, vehicle);
        UIHelper.showSuccess(this, "Driver account created! You can now log in");
    ...
    private JPanel buildTopBar(MainFrame frame) {
        ...
        bar.add(back);
        bar.add(UIHelper.label("Driver Registration", AppColors.FONT_HEADING, AppColors.TEXT_PRIMARY));
        return bar;
    }
}
