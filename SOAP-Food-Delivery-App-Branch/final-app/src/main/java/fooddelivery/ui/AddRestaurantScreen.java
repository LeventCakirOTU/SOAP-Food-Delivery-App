...
public class AddRestaurantScreen extends JPanel {
    ...
    private void handleCreateRestaurant(RestaurantService restaurantService, UserService userService) {
        ...
        // Build and register the restaurant
        currentRestaurant = new Restaurant();
        currentRestaurant.setId("r" + System.currentTimeMillis());
        currentRestaurant.setName(name);
        currentRestaurant.setAddress(address);
        currentRestaurant.setCategory(category);
        currentRestaurant.setOpenHours(hours);
        currentRestaurant.setRating(0.0);

        restaurantService.registerRestaurant(currentRestaurant);

        UIHelper.showSuccess(this, "Restaurant \"" + name + "\" created! You can now add menu items on the right.");
    }
    ...
    private void handleAddMenuItem() {
        ...
        try {
            ...
            // Build MenuItem and add to the current restaurant
            ...
            itemNameField.setForeground(AppColors.TEXT_MUTED);
            itemDescField.setText("Description");
            itemDescField.setForeground(AppColors.TEXT_MUTED);
            itemPriceField.setText("Price (e.g. 9.99)");
